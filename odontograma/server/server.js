  ...
  try {
    ...
    const q = `
      ...
      SELECT
        ...
        od.Fecha_Creacion,
        od.Usuario_Creacion,
        (SELECT COUNT(*) FROM dbo.OdontogramaVersion v WHERE v.OdontogramaId = od.Id) AS Versiones,
        RIGHT('00000' + CAST(od.CorrelativoNum AS VARCHAR(10)), 5) AS Correlativo
      FROM Ordered od
      ORDER BY od.Fecha_Creacion DESC, od.Id DESC`;
...
app.get('/api/odontograma/historico/:nroCuenta/:correlativo', async (req, res) => {
  try {
    ...
    const correlativoNum = parseInt(correlativo, 10); // admite '00001' => 1
    if (!nroHistoria || !correlativoNum || correlativoNum < 1) {
      return res.status(400).json({ error: 'parámetros inválidos' });
    }

    // Resolver correlativo local por historia a Id real
    const r0 = pool.request();
    r0.input('nroHistoria', sql.NVarChar(50), nroHistoria);
    r0.input('correlativoNum', sql.Int, correlativoNum);
    const e = await r0.query(`
      ;WITH Ordered AS (
        SELECT
          o.Id,
          ROW_NUMBER() OVER (
      ...
      SELECT TOP 1 Id
      FROM Ordered
      WHERE CorrelativoNum = @correlativoNum`);
    if (!e.recordset || e.recordset.length === 0) {
      return res.status(404).json({ error: 'Odontograma no encontrado para la cuenta indicada' });
    }
    const id = e.recordset[0].Id;
    ...
    odontograma.correlativo = ('00000' + String(correlativoNum)).slice(-5);
